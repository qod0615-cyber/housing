...
12. **🧲 벽 밀착 상태 가구 간 연동 스냅 & 미세 스냅 반경 (6cm) 적용 완료 (2026-09-15)**
    - **벽 밀착 이동 중 옆 가구 연동 스냅 (Wall + Furniture Combined Snap)**: 가구가 벽면에 붙어있는 상태(`wallSnap.isSnapped`)로 벽을 따라 슬라이딩 이동할 때, 옆에 배치된 가구를 그냥 뚫고 지나치지 않고 옆 가구 외곽선에도 자석으로 찰싹 붙도록 `moveAt` 계산 수정 (`🧲 방 1 왼쪽 벽 & 수납장 하단 밀착` 결합 피드백 제공).
    - **자석 흡착 반경 미세화 (Threshold 6cm)**: 넓은 스냅 범위로 인해 원치 않는 위치로 강제로 붙던 조작 불편감을 해결하기 위해 기본 스냅 반경을 6cm로 축소하여, 실제로 가구가 벽이나 타 가구에 바짝 접근했을 때만 정밀 스냅 및 손쉬운 분리 가능하도록 조절.
    - **PropertyInspector 스냅 정보 카드 업데이트**: 벽 + 가구 이중 스냅 상태를 Inspector 상태 카드 및 피드백 툴팁에 실시간 표시.
    - **Vercel 라이브 배포 완료**: [https://housing-bay.vercel.app](https://housing-bay.vercel.app)
