...
12. **🧲 벽 밀착 상태 가구 연동 스냅 & 비대칭 침범 저항 스냅 (25cm) 구현 완료 (2026-09-15)**
    - **벽 밀착 이동 중 옆 가구 연동 스냅 (Wall + Furniture Combined Snap)**: 가구가 벽면에 붙어있는 상태(`wallSnap.isSnapped`)로 벽을 따라 슬라이딩 이동할 때, 옆에 배치된 가구를 뚫고 지나치지 않고 옆 가구 외곽선에도 자석으로 찰싹 붙도록 연동 스냅 구현 (`🧲 방 1 왼쪽 벽 & 수납장 하단 밀착` 결합 피드백 제공).
    - **비대칭 침범 저항 자석 흡착 (Approach 6cm / Penetration 25cm)**: 
      - **접근/떨어짐 방향 (Approach/Release)**: 6cm 정밀 반경으로 손쉽게 접근 및 분리 가능.
      - **침범/뚫기 방향 (Penetration Resistance)**: 벽이나 타 가구 내부로 밀어 넣으려는 방향으로는 25cm의 넓은 저항 버퍼를 적용하여, 마우스를 벽/가구 안쪽으로 밀어도 물리적으로 더 이상 들어가지 않고 "벽/가구에 막혔음"을 직관적으로 체감할 수 있도록 구현.
    - **Vercel 라이브 배포 완료**: [https://housing-bay.vercel.app](https://housing-bay.vercel.app)
